...
@Component({
  selector: 'app-edit-approval-scheme',
  imports: [SharedModule],
  templateUrl: './edit-approval-scheme.component.html',
  styleUrl: './edit-approval-scheme.component.scss'
})
export class EditApprovalSchemeComponent {
  ...
  constructor(private masterService: MasterService,
    ...
    private dialogRef: MatDialogRef<EditApprovalSchemeComponent>) { }
  ...
  onSubmit() {
    if (this.schemeForm.valid) {
      ...
      this.schemeService.RegisterApprovedScheme(this.schemeForm.value).subscribe(response => {
        if (response.errorMessage != null) {
          // alert(response.errorMessage);
          this.toastr.error(response.errorMessage);
        } else {
          this.dialogRef.close('Scheme Succesfully Approved');
        }
      });
