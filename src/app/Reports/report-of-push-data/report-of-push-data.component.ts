...
export class ReportOfPushDataComponent {
  ...
  loadTableData() {
    ...
    this.dbtDataService.GetReportPushDataPaged(
      ...
      this.dataSource.sortingDataAccessor = (item: any, property: string) => {
        ...
        const toStringLower = (val: any): string => (val ?? '').toString().toLowerCase();
        ...
        switch (property) {
          ...
          case 'add_state_contribution':
            return toNumber(item?.fundCashStateAdditional ?? item?.additionalStateShare);
  ...
  public saveToExcel() {
    ...
    this.dbtDataService.GetReportPushDataPaged(this.DeptCode, this.SchemeCode, this.FinYrCode, this.MonthId, payload).subscribe(x => {
      // console.log(x);
  ...
  searchScheme(e: any) {
    if (e !== undefined) {
      ...
      if (term !== undefined && term !== '' && term != null) {
        if (term.length > 0) {
          this.schemeList = this.copySchemeList.filter((x: any) => x.schemeName.toLowerCase().includes(term.toLowerCase()));
        } else {
  ...
  searchDept(e: any) {
    if (e !== undefined) {
      ...
      if (term !== undefined && term !== '' && term != null) {
        if (term.length > 0) {
          this.departmentList = this.copyDepartmentList.filter((x: any) => x.name.toLowerCase().includes(term.toLowerCase()));
        } else {
